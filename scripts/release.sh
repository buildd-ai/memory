...
### Changes
$(git log "origin/$MAIN_BRANCH..HEAD" --format='- %s' --no-merges 2>/dev/null | grep -v '^- chore: bump version' | head -20)
...
EOF
  ...
  if [ -n "$pr_number" ]; then
    ...
    gh api "repos/$repo/pulls/$pr_number" --method PATCH \
      -f title="Release v$next" -f body="$body" --silent
  else
...
### Changes
$(git log "origin/$MAIN_BRANCH..HEAD" --format='- %s' --no-merges 2>/dev/null | grep -v '^- chore: bump version' | head -30)
